...
def voxelize(points, ids, room_bbox, width, length, height):
    ...
    x = np.clip((points[:, 0] - bbox_min[0]) / (bbox_max[0] - bbox_min[0]) * width, 0, width - 1).astype(np.int32)
    y = np.clip((points[:, 1] - bbox_min[1]) / (bbox_max[1] - bbox_min[1]) * length, 0, length - 1).astype(np.int32)
    z = np.clip((points[:, 2] - bbox_min[2]) / (bbox_max[2] - bbox_min[2]) * height, 0, height - 1).astype(np.int32)
...
if __name__ == '__main__':
    pcd_dir = '/data/bhuai/BlenderProc/FRONT3D_render/pcd'
    feat_dir = '/data/bhuai/3dfront_rpn_data/features_256'
    xform_dir = '/data/bhuai/front3d_ngp'
    out_dir = '/data/bhuai/BlenderProc/FRONT3D_render/voxel'
    
    os.makedirs(out_dir, exist_ok=True)
    scenes = os.listdir(pcd_dir)
    scenes = ['3dfront_0003_01.npz']

    for s in tqdm(scenes):
        scene_name = s.split('.')[0]
        pcd = np.load(os.path.join(pcd_dir, s))
        points = pcd['points']
        ids = pcd['ids']
        ...
        with open(os.path.join(xform_dir, scene_name, 'train', 'transforms.json'), 'r') as f:
            data = json.load(f)
