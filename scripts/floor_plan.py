...
class FloorPlan:
    def __init__(self, scene_idx):
        ...
        (
            ...
            self.bbox_maxs,
            self.loaded_objects,
        ) = get_scene_bbox_meta(scene_idx)

        self.point_sampler = Front3DPointInRoomSampler(self.loaded_objects)

        print("==================FLOOR PLAN INIT===============\n\n\n")
        print("used floors", self.point_sampler.used_floors)
        print("uses floors", random.choice(self.point_sampler.used_floors))

        for i in range(len(self.point_sampler.used_floors)):
            floor_obj = self.point_sampler.used_floors[i]
        ...
        self.width = (
            int((self.scene_max - self.scene_min)[0] * self.scale) + self.margin * 2
        )
        self.height = (
            int((self.scene_max - self.scene_min)[1] * self.scale) + self.margin * 2
        )
    ...
    def draw_coords(self):
        ...
        for i in range(
            int(np.floor(self.scene_min[0])), int(np.ceil(self.scene_max[0]) + 1)
        ):
            cv2.line(
                self.image,
                self.point_to_image([i, -seg]),
                self.point_to_image([i, seg]),
                color=red,
                thickness=2,
            )
        for i in range(
            int(np.floor(self.scene_min[1])), int(np.ceil(self.scene_max[1]) + 1)
        ):
            cv2.line(
                self.image,
                self.point_to_image([-seg, i]),
                self.point_to_image([seg, i]),
                color=red,
                thickness=2,
            )
    ...
    def draw_room_bbox(self):
        if self.scene_idx in ROOM_CONFIG.keys():
            for value in ROOM_CONFIG[self.scene_idx].values():
                ...
                cv2.rectangle(
                    self.image,
                    self.point_to_image(scene_bbox[0]),
                    self.point_to_image(scene_bbox[1]),
                    color=blue,
                    thickness=5,
                )
    ...
    def draw_objects(self):
        ...
        for i in range(len(self.names)):
            ...
            if self.names[i][:4] == "Wall":
                # print("self.names[i]", self.names[i])
                # print("self.bbox_mins[i]", self.bbox_mins[i])
                # print("self.bbox_maxs[i]", self.bbox_maxs[i])
    ...
    def point_to_image(self, point_3d):
        ...
        """
        return int(
            point_3d[0] * self.scale - self.scene_min[0] * self.scale + self.margin
        ), self.height - int(
            point_3d[1] * self.scale - self.scene_min[1] * self.scale + self.margin
        )
...
class FloorPlan_rot:
    ...
    def draw_objects(self):
        for i in range(len(self.names)):
            ...
            if self.names[i][:4] == "Wall":
                cv2.rectangle(
                    self.image, (x_min, y_min), (x_max, y_max), (255, 255, 0), -1
                )
                ...
                for j in range(3):
                    cv2.line(
                        self.image,
                        (x_list[j + 4], y_list[j + 4]),
                        (x_list[j + 5], y_list[j + 5]),
                        color,
                        2,
                    )
                cv2.putText(
